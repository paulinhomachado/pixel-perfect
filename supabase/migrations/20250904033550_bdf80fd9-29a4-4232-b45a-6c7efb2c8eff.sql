...
DROP POLICY IF EXISTS "Administradores e funcionários podem ver funcionários" ON public.funcionarios;
DROP POLICY IF EXISTS "Administradores podem inserir funcionários" ON public.funcionarios;
...
CREATE POLICY "Admin and self access funcionarios" 
...
CREATE POLICY "Admin can insert funcionarios" 
ON public.funcionarios 
FOR INSERT 
WITH CHECK (
...
CREATE POLICY "Admin can update funcionarios" 
ON public.funcionarios 
FOR UPDATE 
USING (
  public.get_current_user_role() = 'administrador'::nivel_acesso
...
CREATE POLICY "Admin can delete funcionarios" 
ON public.funcionarios 
FOR DELETE 
USING (
  public.get_current_user_role() = 'administrador'::nivel_acesso
...
CREATE POLICY "Admin can manage comissoes" 
ON public.comissoes 
FOR ALL 
USING (public.get_current_user_role() = 'administrador'::nivel_acesso);

CREATE POLICY "Employee can view own comissoes" 
ON public.comissoes 
FOR SELECT 
USING (
